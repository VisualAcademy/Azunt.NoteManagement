...
{
    public class NotesTableBuilder
    {
        private readonly string _masterConnectionString;
        private readonly ILogger<NotesTableBuilder> _logger;

        public NotesTableBuilder(string masterConnectionString, ILogger<NotesTableBuilder> logger)
        {
        ...
        public void BuildTenantDatabases()
        {
            ...
            foreach (var connStr in tenantConnectionStrings)
            {
                try
                {
                    EnsureNotesTable(connStr);
                    _logger.LogInformation($"Notes table processed (tenant DB): {connStr}");
                }
                catch (Exception ex)
                {
        ...
        public void BuildMasterDatabase()
        {
            try
            {
                EnsureNotesTable(_masterConnectionString);
                _logger.LogInformation("Notes table processed (master DB)");
            }
            catch (Exception ex)
            {
        ...
        private void EnsureNotesTable(string connectionString)
        {
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                var cmdCheck = new SqlCommand(@"
                    ...
                    WHERE TABLE_NAME = 'Notes'", connection);
                ...
                if (tableCount == 0)
                {
                    ...
                    var cmdCreate = new SqlCommand(@"
                        CREATE TABLE [dbo].[Notes] (
                            ...
                            [Name] NVARCHAR(255) NULL,                                       -- 노트 이름 (255자로 제한)
                    ...
                    _logger.LogInformation("Notes table created.");
                }
                ...
                foreach (var (columnName, columnDefinition) in expectedColumns)
                {
                    var cmdColCheck = new SqlCommand(@"
                        ...
                        WHERE TABLE_NAME = 'Notes' AND COLUMN_NAME = @ColumnName", connection);
                    ...
                    if (columnExists == 0)
                    {
                        ...
                        _logger.LogInformation($"Column added to Notes: {columnName} ({columnDefinition})");
                    }
                }

                // Insert default rows if the table is empty
                var cmdCountRows = new SqlCommand("SELECT COUNT(*) FROM [dbo].[Notes]", connection);
                int rowCount = (int)cmdCountRows.ExecuteScalar();

                if (rowCount == 0)
                {
                    var cmdInsertDefaults = new SqlCommand(@"
                        INSERT INTO [dbo].[Notes] (Active, IsDeleted, Created, CreatedBy, Name, DisplayOrder)
                        VALUES
                            (1, 0, SYSDATETIMEOFFSET(), 'System', 'Initial Note 1', 1),
                            (1, 0, SYSDATETIMEOFFSET(), 'System', 'Initial Note 2', 2)", connection);

                    int inserted = cmdInsertDefaults.ExecuteNonQuery();
                    _logger.LogInformation($"Notes default data inserted: {inserted} rows.");
                }
            }
        }
        ...
        public static void Run(IServiceProvider services, bool forMaster, string? optionalConnectionString = null)
        {
            try
            {
                var logger = services.GetRequiredService<ILogger<NotesTableBuilder>>();
                ...
                var builder = new NotesTableBuilder(connectionString, logger);
            ...
            {
                var fallbackLogger = services.GetService<ILogger<NotesTableBuilder>>();
                fallbackLogger?.LogError(ex, "Error while processing Notes table.");
            }
        }
    }
}