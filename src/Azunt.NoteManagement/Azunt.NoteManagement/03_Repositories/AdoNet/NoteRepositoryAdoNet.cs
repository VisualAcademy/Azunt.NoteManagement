...
public class NoteRepositoryAdoNet : INoteRepository
{
    ...
    private readonly ILogger<NoteRepositoryAdoNet> _logger;

    public NoteRepositoryAdoNet(string connectionString, ILoggerFactory loggerFactory)
    {
        _connectionString = connectionString;
        _logger = loggerFactory.CreateLogger<NoteRepositoryAdoNet>();
    }
    ...
    public async Task<Note> AddAsync(Note model)
    {
        ...
        if (result == null)
        {
            throw new InvalidOperationException("Failed to insert Note. No ID was returned.");
        }
    ...
    public async Task<IEnumerable<Note>> GetAllAsync()
    {
        ...
        using var conn = GetConnection();
        using var cmd = conn.CreateCommand();
        cmd.CommandText = "SELECT Id, Active, Created, CreatedBy, Name, Category FROM Notes WHERE IsDeleted = 0 ORDER BY DisplayOrder";
    ...
    public async Task<Note> GetByIdAsync(long id)
    {
        using var conn = GetConnection();
        using var cmd = conn.CreateCommand();
        cmd.CommandText = "SELECT Id, Active, Created, CreatedBy, Name, Category FROM Notes WHERE Id = @Id AND IsDeleted = 0";
    ...
    public async Task<bool> DeleteAsync(long id)
    {
        using var conn = GetConnection();
        using var cmd = conn.CreateCommand();
        cmd.CommandText = "UPDATE Notes SET IsDeleted = 1 WHERE Id = @Id AND IsDeleted = 0";
        cmd.Parameters.AddWithValue("@Id", id);
    ...
    public async Task<bool> MoveUpAsync(long id)
    {
        ...
        cmd1.CommandText = "SELECT Id, DisplayOrder FROM Notes WHERE Id = @Id AND IsDeleted = 0";
        ...
        try
        {
            ...
            cmdUpdate1.CommandText = "UPDATE Notes SET DisplayOrder = @NewOrder WHERE Id = @Id";
            ...
            cmdUpdate2.CommandText = "UPDATE Notes SET DisplayOrder = @NewOrder WHERE Id = @Id";
